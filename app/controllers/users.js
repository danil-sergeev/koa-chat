...
export const getAllUsers = () => {
	return User.find().select('username firstName lastName').exec().then((users) => {
...
export const getUserDataById = (userId) => {
	return User.findById(userId).select('username firstName lastName').lean().exec();
};
...
const getUserMessagesCountById = (userId) => {
	return Message.find({ sender: userId, statusMessage: false }).then((messages) => messages.count || 0);
};

export const getUserInfo = (userId) => {
	const userPromises = [ getUserDataById(userId), getUserChatsById(userId), getUserMessagesCountById(userId) ];
	console.log('kek');
